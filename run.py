...
=======================================
Pipeline FULLY AUTOMATED: Research → Narasi → YouTube + AI Scene Match → Render via API

Cara pakai:
  python run.py --prompt "kenapa rupiah melemah" --template edukasi-shorts
...
def _aspect_from_format(fmt: str) -> str:
    return _VALIDATE_ASPECTS.get(fmt, "9:16")


def _build_video_materials(matched_scenes: list, output_dir: str = None) -> list:
    """
    Convert matched scene clips → MaterialInfo format untuk API.

    Backend mewajibkan file di storage/local_videos/ (security check).
    Fungsi ini auto-copy clip ke sana dan return relative path.
    """
    local_videos_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
    ...
    for m in matched_scenes:
        ...
        if not clip_path or not os.path.exists(clip_path):
            logger.warning(f"   ⚠️  Clip path not found: {clip_path}")
            continue

        dur = int(m.get("duration", 5))
        if dur <= 0:
            continue

        # Copy ke storage/local_videos/ biar lolos security check backend
        basename = os.path.basename(clip_path)
        dest = os.path.join(local_videos_dir, basename)

        # Only copy if not already there
        if not os.path.exists(dest):
            _copy_file_with_retry(clip_path, dest, max_retries=3)
            logger.debug(f"   📋 Copied: {basename} → storage/local_videos/")

        materials.append({
            "provider": "local",
            "url": basename,  # relative — backend resolve dari local_videos_dir
            "duration": dur,
        })

    if not materials:
        logger.warning("⚠️  No valid video materials — will use Pexels fallback")
        return None

    logger.info(f"   📦 {len(materials)} video materials ready")
    return materials


def _copy_file_with_retry(src: str, dst: str, max_retries: int = 3) -> bool:
...
def create_video_task(script: str, subject: str, terms: list, template: dict, fmt: str,
                      video_materials: list = None) -> dict:
    """
    Call POST /api/v1/videos to create a video generation task
    
    Args:
        video_materials: List of {provider, url, duration} — dari YouTube clips
    ...
    body = {
        ...
        "video_count": 1,
        "video_source": "pexels" if not video_materials else "local",
        ...
        "font_name": template.get("subtitle", {}).get("font", "BeVietnamPro-Bold.ttf"),
        ...
        "match_materials_to_script": bool(video_materials),
    }

    logger.info(f"🎬 Mengirim task ke API...")
    if video_materials:
        logger.info(f"   📦 {len(video_materials)} video materials (YouTube clips)")
    ...
    try:
        ...
        if result.get("status") == 200:
            ...
            # If materials not supported, retry without them
            if video_materials and ("materials" in str(error_msg).lower() or "source" in str(error_msg).lower()):
                logger.warning("⚠️ Materials not supported by API, retrying with Pexels only")
                body["video_source"] = "pexels"
                body["video_materials"] = None
                r = requests.post(f"{API_BASE}/videos", json=body, timeout=10)
                result = r.json()
                if result.get("status") == 200:
                    task_id = result.get("data", {}).get("task_id", "")
                    logger.success(f"✅ Task created (Pexels fallback): {task_id}")
                    return {"task_id": task_id, "success": True}
...
Contoh:
  ...
  python run.py --extract-template "https://youtu.be/vKVlghqgjg0"
    ...
    parser.add_argument("--skip-youtube", action="store_true",
                        help="Skip YouTube search & download (default: AKTIF)")
    parser.add_argument("--no-ai-matcher", action="store_true",
                        help="Skip AI scene matcher, pake keyword fallback")
    ...
    # ═══════════════════════════════════════════════════════════════
    # STEP 3: YOUTUBE SEARCH + DOWNLOAD + AI SCENE MATCH (AKTIF!)
    # ═══════════════════════════════════════════════════════════════
    downloaded_videos = []
    matched_scenes = []
    ...
    if not args.skip_youtube:
        logger.info("")
        logger.info("═" * 50)
        logger.info("🎬 [3/4] YOUTUBE — Mencari video pendukung + AI Matching")
        logger.info("═" * 50)

        yt_search = args.yt_keywords or search_terms[:3]
        yt_output_dir = os.path.join(output_dir, "yt_downloads")

        if args.yt_urls:
            for url in args.yt_urls:
                vpath = yt.download_video(url, yt_output_dir)
                if vpath:
                    downloaded_videos.append(vpath)
        else:
            for kw in yt_search:
                videos = yt.search_youtube(kw, max_results=2)
                for v in videos:
                    if len(downloaded_videos) >= 5:
                        break
                    logger.info(f"   📥 Download: {v['title']} ({v['duration_str']})")
                    vpath = yt.download_video(v["url"], yt_output_dir)
                    if vpath:
                        downloaded_videos.append(vpath)

        logger.info(f"📦 Total YouTube clips: {len(downloaded_videos)}")

        # ── Scene Detection on YouTube clips ──
        if downloaded_videos:
            logger.info("")
            logger.info("🔍 Scene detection on YouTube clips...")
            all_clips = []
            for vpath in downloaded_videos:
                # Detect scenes in downloaded video
                scenes_in_video = yt.detect_scenes(vpath, threshold=25.0)
                if scenes_in_video:
                    # Extract each scene as a separate clip
                    scene_clips = yt.extract_scene_clips(
                        vpath, scenes_in_video,
                        os.path.join(yt_output_dir, "scenes"),
                        min_duration=2.0, max_duration=15.0
                    )
                    all_clips.extend(scene_clips)
                    logger.info(f"   🎬 {os.path.basename(vpath)}: {len(scene_clips)} scene clips")
                else:
                    # Fallback: use full video as one clip
                    dur = yt._get_clip_duration(vpath)
                    all_clips.append({
                        "path": vpath,
                        "start_time": 0,
                        "duration": dur,
            ...
            logger.info(f"📦 Total scene clips after detection: {len(all_clips)}")

            # ── AI Scene Matching ──
            if all_clips and narration.get("scenes"):
                clip_paths = [c["path"] for c in all_clips if os.path.exists(c.get("path", ""))]

                if clip_paths:
                    if not args.no_ai_matcher:
                        matched_scenes = matcher.match_scenes_to_clips_ai(
                            narration["scenes"], clip_paths, prompt
                        )
                    else:
                        matched_scenes = matcher.match_scenes_to_clips_keyword(
                            narration["scenes"], clip_paths, prompt
                        )

                    # Save match results
                    with open(os.path.join(output_dir, "matches.json"), "w", encoding="utf-8") as f:
                        json.dump(matched_scenes, f, indent=2, ensure_ascii=False)

        if not downloaded_videos:
            logger.info("   ⚠️  YouTube skip — pake Pexels footage aja")

    # ═══════════════════════════════════════════════════════════════
    # STEP 4: RENDER VIA API (dengan YouTube clips jika ada)
    # ═══════════════════════════════════════════════════════════════
    logger.info("")
    logger.info("═" * 50)
    ...
    # Build video_materials from matched scenes (jika ada)
    video_materials = _build_video_materials(matched_scenes) if matched_scenes else None

    if video_materials:
        logger.info(f"📦 Injecting {len(video_materials)} matched clips ke API")
    ...
    summary = {
        ...
        "videos": downloaded,
        "task_id": task_id,
        "scenes": len(narration.get("scenes", [])),
        "youtube_clips": len(downloaded_videos),
        "ai_matched": len(matched_scenes),
        "using_youtube_footage": bool(video_materials),
    }
